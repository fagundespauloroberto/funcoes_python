...
def validar_senha(senha, comprimento_min=10, comprimento_max=16): #Valida a senha e retorna a força de 0 a 5.    
    
    palavra_lista = procurar_palavra(senha, "E:/funcoes_python/lista_de_palavras.txt") # 1. Verificar se a senha é uma palavra comum conforme a lista de palavras. Se for, a senha é considerada insegura, mesmo que atenda aos outros requisitos.
    senha_comum = procurar_palavra(senha, "E:/funcoes_python/senhas_mais_comuns.txt") # Verificar se a senha é uma senha comum conforme a lista de senhas mais comuns. Se for, a senha é considerada insegura, mesmo que atenda aos outros requisitos.
    if palavra_lista:
        ...
        return 0
    else: # A senha não é uma palavra comum ou senha comum. Continuamos com a validação...
        tamanho = len(senha) # 2. Verificar Comprimento
        if tamanho < comprimento_min:
            print(f"Aviso: Senha muito curta (mínimo {comprimento_min} caracteres).")
            return 1
        
        complexidade = calcular_complexidade(senha) # 3. Calcular Complexidade        
        # Lógica de Força Final:        
        if tamanho >= comprimento_max and complexidade == 4: # Se for maior que o comprimento forte e tiver complexidade 4 -> Força 5
...
def main():
    print("Bem-vindo ao VALIDADOR DE SENHAS!")
    print("Para sair a qualquer momento, digite a letra 'q' ou 'Q'.")
    while True:
